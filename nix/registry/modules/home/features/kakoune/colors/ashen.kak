...
# Mode-aware cursor and status line colors
...
declare-option str ashen_statusline_normal '{%opt{background},%opt{orange_blaze}+b}  NORMAL  {%opt{g_3},%opt{g_9}} %val{cursor_line}:%val{cursor_char_column}  %val{bufname}  {%opt{g_3},%opt{g_9}+bi}kak  '
declare-option str ashen_statusline_insert '{%opt{background},%opt{g_3}+b}  INSERT  {%opt{g_3},%opt{g_9}} %val{cursor_line}:%val{cursor_char_column}  %val{bufname}  {%opt{g_3},%opt{g_9}+bi}kak  '
declare-option str ashen_statusline_prompt '{%opt{background},%opt{orange_glow}+b}  COMMAND  {%opt{g_3},%opt{g_9}} %val{cursor_line}:%val{cursor_char_column}  %val{bufname}  {%opt{g_3},%opt{g_9}+bi}kak  '

define-command -hidden ashen-apply-normal %{
  set-face global PrimaryCursor      "%opt{background},%opt{orange_blaze}"
  set-face global PrimaryCursorEol   "%opt{background},%opt{orange_blaze}"
  set-face global SecondaryCursor    "%opt{background},%opt{orange_muted}"
  set-face global SecondaryCursorEol "%opt{background},%opt{orange_muted}"
  set-face global StatusCursor       "%opt{background},%opt{orange_blaze}"
  set-face global LineNumberCursor   "%opt{orange_blaze},%opt{background}+b"
  set-face global crosshairs_line    "default,%opt{cursor_line_normal}"
  set-face global StatusLineMode     "%opt{background},%opt{orange_blaze}"
  set-face global Prompt             "%opt{orange_glow}"
  set-option global modelinefmt "%opt{ashen_statusline_normal}"
}

define-command -hidden ashen-apply-insert %{
  set-face global PrimaryCursor      "%opt{background},%opt{g_3}"
  set-face global PrimaryCursorEol   "%opt{background},%opt{g_3}"
  set-face global SecondaryCursor    "%opt{background},%opt{g_7}"
  set-face global SecondaryCursorEol "%opt{background},%opt{g_7}"
  set-face global StatusCursor       "%opt{background},%opt{g_3}"
  set-face global LineNumberCursor   "%opt{g_3},%opt{background}+b"
  set-face global crosshairs_line    "default,%opt{cursor_line_insert}"
  set-face global StatusLineMode     "%opt{background},%opt{g_3}"
  set-face global Prompt             "%opt{orange_glow}"
  set-option global modelinefmt "%opt{ashen_statusline_insert}"
}

define-command -hidden ashen-apply-prompt %{
  set-face global PrimaryCursor      "%opt{background},%opt{orange_glow}"
  set-face global PrimaryCursorEol   "%opt{background},%opt{orange_glow}"
  set-face global SecondaryCursor    "%opt{background},%opt{orange_muted}"
  set-face global SecondaryCursorEol "%opt{background},%opt{orange_muted}"
  set-face global StatusCursor       "%opt{background},%opt{orange_glow}"
  set-face global LineNumberCursor   "%opt{orange_glow},%opt{background}+b"
  set-face global crosshairs_line    "default,%opt{cursor_line_prompt}"
  set-face global StatusLineMode     "%opt{background},%opt{orange_glow}"
  set-face global Prompt             "%opt{background},%opt{orange_glow}+b"
  set-option global modelinefmt "%opt{ashen_statusline_prompt}"
}

remove-hooks global ashen-mode
hook -group ashen-mode global ModeChange (push|pop):.*insert %{ ashen-apply-insert }
hook -group ashen-mode global ModeChange (push|pop):insert:.* %{ ashen-apply-normal }
hook -group ashen-mode global ModeChange (push|pop):.*prompt %{ ashen-apply-prompt }
hook -group ashen-mode global ModeChange (push|pop):prompt.* %{ ashen-apply-normal }
hook -group ashen-mode global KakBegin .* %{ ashen-apply-normal }
ashen-apply-normal
