...
let
  ...
  mergeKeymapSets = lhs: rhs:
    if lhs == null then rhs
    else if rhs == null then lhs
    else if lib.isList lhs && lib.isList rhs then lhs ++ rhs
    else if lib.isAttrs lhs && lib.isAttrs rhs then
      let keys = lib.unique ((builtins.attrNames lhs) ++ (builtins.attrNames rhs));
      in lib.genAttrs keys (key: mergeKeymapSets (lib.attrByPath [ key ] null lhs) (lib.attrByPath [ key ] null rhs))
    else rhs;
  ...
  pluginsLua = lib.generators.toLua { multiline = true; indent = "  "; } pluginNames;
  pluginOptsLua = if pluginSetupOptions == { } then "{}"
    else lib.generators.toLua { multiline = true; indent = "  "; } pluginSetupOptions;
  ...
  extraPackages = lib.unique (
    (with pkgs; [ mediainfo ffmpeg ]) ++ pluginExtraPackages
  );
in
{
  programs.yazi = {
